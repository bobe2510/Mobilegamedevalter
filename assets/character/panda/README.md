...
補單格時可以用 `--seed` 換一個骰（同一個 seed 重產結果一樣）。

## 壞格會自動被抓出來

`tilt` 第一次產出來**少了一隻耳朵和一個黑眼圈**，整顆頭變成白球。
這是眼睛抓到的、不是工具抓到的——所以 `build_character.py` 加了自動檢查：
比對每一格跟基準格的用色比例，差太多就示警。

門檻是**量出來的**：熊貓十格裡，正常格跟基準格的差異最大 0.060
（`pull` 四足、`jump` 騰空這種姿勢差很多的也才 0.052~0.060），
而壞掉的 `tilt` 是 0.154。中間有 2.5 倍間隔，所以取 0.10。

抓得到「某個部位整個不見了」；抓不到「畫錯但用色正常」，那種還是要看。
